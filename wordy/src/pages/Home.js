...
export const Home = ({favoriteList, setFavorites, word, setWord}) => {
  ...
  return (
    <section className="absolute z-10 h-full w-full flex flex-col justify-center items-center">
        <div className="h-auto w-[750px] mb-[5em] p-6">
            <form onSubmit={handleSumbit} className="flex items-center">   
                ...
                <div className="relative w-full">
                    ...
                    <input value={word.word ? word.word : ""} onChange={(event) => {setWord({...word, word: event.target.value })}} type="text" name="word" id="simple-search" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for Word..." autoComplete="off" required/>
                </div>
        ...
        <FavoriteList  favoriteList={favoriteList} setFavorites={setFavorites} setWord={setWord}/>
